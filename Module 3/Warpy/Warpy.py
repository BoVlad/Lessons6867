...
playercountrymove = ctk.CTkLabel(root, font=("Arial", 20, "bold"), width=280, height=55)
    ...
    def newgamebutton1def():
        ...
        if year == "Друга світова війна":
            ...
            def nextWW2(choice):
                ...
                playercountrymove.configure(text=f"Обрана дія\nПорожньо")
                playercountrymove.place(relx=0.5, rely=0.045, anchor="center")
...
def on_left_click(indexdef):
    global playercountry, buttonslist, currentaction, currentactionin4, tempcolor, mainbuttonindex

    def commandplayer(commandnumb):
        global playercountry, buttonslist
        if commandnumb == 0:
        ...
        if commandnumb == 5:
            global currentaction, mainbuttonindex, slidervalue, tempcolor
            if buttonslist[indexdef].info.get("Army") > 0:
                if buttonslist[indexdef].info.get("Canmove"):
                    mainbuttonindex = indexdef
                    tempcolor = buttonslist[mainbuttonindex].cget("fg_color")


                    buttonslist[mainbuttonindex].configure(fg_color="orange red")
                    playercountrymove.configure(
                        text=f"Обрана дія (для відміни натисніть на ту саму кнопку)\nПеревезти воїнів на іншу позицію",
                        font=("Arial", 10, "bold"))
                    buttonslist[mainbuttonindex].configure(fg_color="orange red")
                    currentaction = True
                else:
    ...
    if not currentaction:
        ...
        if buttonslist[indexdef].info.get("Playercountrybtn"):
            if buttonslist[indexdef].info.get("Army") < 10000:
                if indexdef == mainbuttonindex:
                    currentaction = False
                    buttonslist[mainbuttonindex].configure(fg_color=tempcolor)
                    playercountrymove.configure(text=f"Обрана дія\nПорожньо")
                    return
                else:
                    def confirmplayer5():
                        global slidervalue, currentaction, mainbuttonindex
                        sliderslider.place_forget()
                        sliderbuttonlabel.place_forget()
                        sliderbuttonconfirm.place_forget()
                        sliderbuttonexit.place_forget()
                        sliderframe.place_forget()
                        slidervalue = int(sliderslider.get())
                        buttonslist[indexdef].info["Army"] = buttonslist[indexdef].info.get("Army") - int(
                            sliderslider.get())
                        if buttonslist[mainbuttonindex].info.get("Army") > 0:
                            buttonslist[mainbuttonindex].configure(text=f"{buttonslist[mainbuttonindex].info.get("Country")}\n{buttonslist[mainbuttonindex].info.get("Army")}")
                        else:
                            buttonslist[mainbuttonindex].configure(text=f"{buttonslist[mainbuttonindex].info.get("Country")}")
                    ...
                    sliderslider.place(relx=0.5, rely=0.5, anchor="center")
                    sliderbuttonlabel.place(relx=0.5, rely=0.6, anchor="center")
                    sliderbuttonconfirm.configure(command=confirmplayer5)
                    sliderbuttonconfirm.place(relx=0.5, rely=0.7, anchor="center")
                    sliderbuttonexit.configure(command=exitbutton)
                    sliderbuttonexit.place(relx=0.5, rely=0.9, anchor="center")
                    sliderframe.configure(height=400, width=700)
