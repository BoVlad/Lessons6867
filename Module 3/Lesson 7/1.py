...
def on_button_click(text):
    ...
    try:
        if text == "=":
            alleval = eval(display.get())
            display.delete(0, tk.END)
            display.insert(tk.END, str(alleval))
        elif text == "C":
            display.delete(0, tk.END)
        elif text == "←":
            text1last = display.get()
            display.delete(0, tk.END)
            display.insert(0, text1last[:-1])
        else:
            display.insert(tk.END, text)
    except ZeroDivisionError:
        display.delete(0, tk.END)
        label.config(text="Помилка: ділити на нуль не можна")
        label.place(relx=0.5, rely=0.8, anchor="center")
    except NameError:
        label.config(text="Помилка: приберіть букви")
        label.place(relx=0.5, rely=0.8, anchor="center")
    except SyntaxError:
        label.config(text="Помилка: закінчіть вираз")
        label.place(relx=0.5, rely=0.8, anchor="center")
